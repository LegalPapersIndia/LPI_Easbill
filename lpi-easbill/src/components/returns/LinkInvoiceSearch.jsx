...
//   const numberField = type === "sales" ? "invoiceNo" : "purchaseNo";
//   const partyIdField = type === "sales" ? "customerId" : "supplierId";
//   const getPartyName = (inv) => inv[partyIdField]?.name || "N/A";

//   const filtered = invoices.filter((inv) =>
//     inv[numberField].toLowerCase().includes(query.toLowerCase()) ||
//     getPartyName(inv).toLowerCase().includes(query.toLowerCase())
//   );
...
//           <div>
//             <p className="text-sm font-medium text-ink tabular-num">{selectedInvoice[numberField]}</p>
//             <p className="text-xs text-ink-muted">{getPartyName(selectedInvoice)} · ₹{selectedInvoice.grandTotal.toLocaleString("en-IN")}</p>
//           </div>
//         </div>
//         <button onClick={() => onSelect(null)} className="text-ink-muted hover:text-status-overdue">
//           <X size={16} />
...
//               >
//                 <p className="text-sm font-medium text-ink tabular-num">{inv[numberField]}</p>
//                 <p className="text-xs text-ink-muted">{getPartyName(inv)} · ₹{inv.grandTotal.toLocaleString("en-IN")}</p>
//               </button>
//             ))
//           )}
//         </div>
    ...
    return (
      <div className="flex items-center justify-between bg-brand-light border border-brand/20 rounded-lg px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-ink tabular-num">{selectedInvoice[numberField]}</p>
            <p className="text-xs text-ink-muted">{selectedInvoice[partyField] || "N/A"} · ₹{selectedInvoice.grandTotal.toLocaleString("en-IN")}</p>
          </div>
        </div>
        <button onClick={() => onSelect(null)} className="text-ink-muted hover:text-status-overdue">
          <X size={16} />
        ...
        <div className="absolute z-10 w-full mt-1 bg-white border border-border rounded-lg shadow-lg max-h-52 overflow-y-auto">
          {filtered.length === 0 ? (
            ...
            filtered.map((inv) => (
              ...
              >
                <p className="text-sm font-medium text-ink tabular-num">{inv[numberField]}</p>
                <p className="text-xs text-ink-muted">{inv[partyField] || "N/A"} · ₹{inv.grandTotal.toLocaleString("en-IN")}</p>
              </button>
            ))
          )}
        </div>
