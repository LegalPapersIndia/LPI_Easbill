...
export default function SalesInvoiceListTable({ invoices, onEdit, onDelete }) {
  ...
  const EXCEL_HEADERS = [
    "Invoice No", "Invoice Date", "Due Date", "Customer Name", "GSTIN",
    "Mobile", "State", "Payment Mode", "Item Name", "HSN Code", "Qty",
    "Unit", "Rate", "GST %", "Amount", "Subtotal", "Discount", "Shipping",
    "Total GST", "Grand Total", "Amount Received", "Balance", "Notes", "Terms",
  ];

  const buildFlatRows = (fullInvoices) => {
    const rows = [];
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const rows = [EXCEL_HEADERS, ...buildFlatRows(fullInvoices)];
  ...
  const handleExportPDF = async () => {
    try {
      ...
      const head = [[
        "Invoice No", "Date", "Due Date", "Customer", "GSTIN", "Mobile",
        "State", "Pay Mode", "Item", "HSN", "Qty", "Unit", "Rate", "GST%",
        "Amount", "Subtotal", "Discount", "Shipping", "GST", "Grand Total",
        "Received", "Balance", "Notes", "Terms",
      ]];
      ...
      fullInvoices.forEach((inv) => {
        ...
        lines.forEach((line) => {
          ...
          body.push([
            ...
            `Rs ${(inv.discount || 0).toLocaleString("en-IN")}`,
            `Rs ${(inv.shipping || 0).toLocaleString("en-IN")}`,
            `Rs ${(inv.gstBreakup || 0).toLocaleString("en-IN")}`,
            `Rs ${inv.grandTotal.toLocaleString("en-IN")}`,
